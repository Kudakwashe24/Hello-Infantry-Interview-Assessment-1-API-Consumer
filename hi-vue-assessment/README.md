# Hello Infantry — Vue Developer Assessment

## Overview

Build a **Vue 3** application that consumes a public API and displays the data with a focus on **performance and optimisation**. This assessment evaluates how you handle real-world frontend challenges — not just "does it work", but "does it work _well_".
...
5. **Code splitting** — use `defineAsyncComponent()` and dynamic `import()` to split your bundle
6. **Minimal re-renders** — use `v-memo`, `shallowRef`, `computed` where appropriate
...
src/
├── main.js                # Entry point
├── App.vue                # RouterView + Suspense wrapper
├── assets/
│   └── main.css           # Global styles
├── components/
│   └── PlaceholderCard.vue # Starter component — replace with your own
├── pages/
│   ├── HomePage.vue       # Home page — TODO comments inside
│   └── DetailPage.vue     # Detail page — TODO comments inside
├── services/
│   └── api.js             # API stubs — implement your API calls here
├── composables/
│   └── useDebounce.js     # Debounce composable stub — implement this
├── router/
│   └── index.js           # Route definitions — convert to lazy imports
└── utils/
...
- **No UI component libraries** (no Vuetify, Element Plus, PrimeVue, etc.) — we want to see _your_ components and CSS
